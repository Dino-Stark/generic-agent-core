...
18. **Complex ReAct debugging scenario optimized**: Example25 now uses a fast-path single-fixer ReAct loop on `InvoiceSummaryEngine.java` with lower turn/token budgets, concise prompt constraints, behavior-gated retries (max two attempts), and deterministic fallback patching so runtime verification converges quickly while still using real streaming LLM execution; both Example24/25 load buggy and verifier sources from `src/test/resources/inputs` on each test run.
...
- **Example25ComplexReActDebugFixTest**: Demonstrates a faster ReAct debugging pipeline that fixes multiple logical defects (iteration, tax rate, rounding) with a compact fixer loop, runtime verification, and deterministic fallback to guarantee completion.
